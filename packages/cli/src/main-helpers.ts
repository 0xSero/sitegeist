...
function getNativeHostManifestPaths(): string[] {
	...
	return dirs.map((d) => path.join(d, "com.sitegeist.bridge.json"));
}
...
export function installNativeHostManifest(extensionId: string): string[] {
	...
	const manifest = {
		name: "com.sitegeist.bridge",
		description: "Sitegeist CLI — zero-config browser control",
		path: absPath,
		type: "stdio",
...
export async function runInitFlow({
	...
	if (!extensionId) {
		console.log("[init] Scanning for Sitegeist extension...");
		extensionId = detectExtensionId();
	}
	if (!extensionId) {
		console.error(
			"[init] Could not detect Sitegeist extension ID.\n" +
	...
	console.log("       Then run: sitegeist status");
}
