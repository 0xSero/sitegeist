	...
	while (Date.now() < deadline) {
		try {
			...
			if (
				...
				agents.some((agent) => String(agent?.name || "") === "sitegeist-electron-agent")
			) {
				return true;
			}
		} catch {}
...
const cmdStatus = async () => {
	...
	try {
		...
		agents = Array.isArray(list) ? list.filter((a) => String(a?.name || "") === "sitegeist-electron-agent") : [];
	} catch (error) {
...
const cmdStart = async () => {
	...
	const child = spawn(process.execPath, [electronAgentPath], {
		...
		env: {
			...process.env,
			SITEGEIST_RELAY_TOKEN: relayConfig.token,
			SITEGEIST_RELAY_URL: relayConfig.relayUrl,
		},
	});
