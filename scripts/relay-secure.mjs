...
const stateDir = path.join(os.homedir(), ".sitegeist");
...
const printEnv = (config) => {
	console.log(`export SITEGEIST_RELAY_TOKEN="${config.token}"`);
	console.log(`export SITEGEIST_RELAY_HOST="${config.host}"`);
	console.log(`export SITEGEIST_RELAY_PORT="${config.port}"`);
};
...
const cmdStart = async (flags) => {
	...
	const child = spawn(process.execPath, [daemonPath], {
		...
		env: {
			...process.env,
			SITEGEIST_RELAY_TOKEN: config.token,
			SITEGEIST_RELAY_HOST: config.host,
			SITEGEIST_RELAY_PORT: String(config.port),
		},
	});
	child.unref();
